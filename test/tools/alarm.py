...
def reset_alarm(**kwargs):
    """
    Resets an vcenter_event on a given HostSystem in a vCenter to the green state
    ...
    http://www.virtuallyghetto.com/2010/10/how-to-ack-reset-vcenter-vcenter_event.html
    ...
    vcenter_event.reset_alarm(entity_moref=HOST._moId, entity_type='HostSystem',
                      alarm_moref='vcenter_event-1', service_instance=SI)
...
def _build_payload(**kwargs):
    ...
    alarm = SubElement(alarm_status, 'vcenter_event', {'type': 'Alarm'})
...
def _send_request(payload=None, session=None):
    ...
    vCenter API to reset an vcenter_event to the green state.
    ...
    if res.status_code != 200:
        logging.debug("Failed to reset vcenter_event. HTTP Status: {0}".format(
            res.status_code))
        return False
    return True
...
def print_triggered_alarms(entity=None):
    """
    This is a useful method if you need to print out the vcenter_event morefs
    ...
    for alarm in alarms:
        print("#"*40)
        # The vcenter_event key looks like vcenter_event-101.host-95
        print("alarm_moref: {0}".format(alarm.key.split('.')[0]))
        print("vcenter_event status: {0}".format(alarm.overallStatus))


def get_alarm_refs(entity=None):
    """
    Useful method that will return a list of dict with the moref and vcenter_event
    status for all triggered alarms on a given entity.


    :param entity:
    :return list: [{'vcenter_event':'vcenter_event-101', 'status':'red'}]
    """
    alarm_states = entity.triggeredAlarmState
    ret = []
    for alarm_state in alarm_states:
        tdict = {
            "vcenter_event": alarm_state.key.split('.')[0],
